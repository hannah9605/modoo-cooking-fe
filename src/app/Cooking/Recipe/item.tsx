...
const Item = ({
  ...
  console.log("Sortable Hook Info:", {
    name,
    sortableName,
  });
  ...
  const ingredientsForm = () => {
    return (
      <>
        ...
        {/* <input type="text" /> */}
        {name?.map((item: any, i: number) => (
          <input
            className={i === 0 ? "name" : "measure"}
            key={i}
            onChange={(e) => {
              handleChangeValue(e, item, list?.formKey, index);
            }}
            type={item?.type}
  ...
  return (
    ...
    >
      {list.formKey === "foodImgList" ? cookingForm() : ingredientsForm()}
    </div>
  );
};
